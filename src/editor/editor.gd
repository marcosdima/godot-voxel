...
func _build_grid() -> void:
	...
	# Check if cells are already initialized, if not create them.
	for layer in range(data.layers):
		for i in range(layer_cells):
			var cell_data = data.cells[i] if i < data.cells.size() else null
			if cell_data == null:
...
func _get_cell_index(i: int) -> int:
	return int(current_layer.value) * layer_cells + i
